...
import { readableStreamToString, AppwriteClientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new AppwriteClientError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new AppwriteClientError(e, e.response?.data)
                }
                if (innerError instanceof AppwriteClientError) {
                    // Got "AppwriteClientError" from the above try block
                    throw innerError;
                }
